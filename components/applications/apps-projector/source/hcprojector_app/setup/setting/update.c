...
static struct completion sys_upg_usb_check_completion;
static int sys_upg_usb_check_task_run = 0;
#endif
...
static bool find_software(DIR *d,  char str[160]){
    ...
    char file_name_lower[160] = {0};
    while((file = readdir(d)) != NULL){
            if(strncmp(file->d_name, ".", 1) == 0){
                continue;
            }
            memset(file_name, 0, 160);
            memset(file_name_lower, 0, 160);
            strcat(file_name, MOUNT_ROOT_DIR);
            strcat(file_name, "/");
            strcat(file_name, file->d_name);
            strcat(file_name_lower, file_name);
            strcat(file_name, "/HCFOTA.bin");
            strcat(file_name_lower, "/hcfota.bin");
            if(!stat(file_name, &sd) || !stat(file_name_lower, &sd)){
...
static void *sys_upg_usb_task(void *arg){
    //char urls[160]={0};
    while(sys_upg_usb_check_task_run){
        wait_for_completion(&sys_upg_usb_check_completion);
        DIR *d = opendir(MOUNT_ROOT_DIR);
        if(find_software(d, urls)){
            upgrade_entry_init();
            control_msg_t msg = {0};
...
int sys_upg_usb_check_notify(){
    complete(&sys_upg_usb_check_completion);
}
...
int sys_upg_usb_check_init(void)
{
    ...
    init_completion(&sys_upg_usb_check_completion);
    sys_upg_usb_check_task_run = 1;
